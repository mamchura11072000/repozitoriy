...
//--------------------------------------------------------------------------------------------------------------------
 	public function getPDOAll (){
   		...
   		$stmt = self::$db->prepare("SELECT dd_user_cards.*, dd_user_person_data.* FROM dd_user_cards JOIN dd_user_person_data ON dd_user_cards.id = dd_user_person_data.id WHERE id=:id");
